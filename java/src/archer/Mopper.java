...
public class Mopper extends Robot {
    ...
    public static final int ATTACK_DIS = 2; //sqrt 2
    public static final int VISION_DIS = 20; //sqrt 20
    private static final int HEALING_CUTOFF = 20;
    ...
    static void play () throws GameActionException {
        // Me: This honestly isn't the best for moppers since we can't attack towers
        if (turnCount == 0) {
...
//        }
        ...
        if (!Comm.isSymmetryConfirmed) {
            ...
            if (rc.canSenseLocation(symGuessLoc)) {
                ...
                if (hq == null || !hq.isTowerType() || hq.team != oppTeam) {
    ...
    static void sense() throws GameActionException {
        anchoringSoldier = null;  //Me: HIGHLY DOUBT THIS WORKS> SHOULD IT BE TOWERS INSTEAD
        attackTarget = null;
        chaseTarget = null;
        groupingTarget = null;
        backupTarget = null;
        ...
        for (RobotInfo robot : rc.senseNearbyRobots()) {
            if (robot.team == myTeam) {
                if (robot.type == UnitType.MOPPER) {
                    if (friendlyMopperCnt >= MAX_FRIENDLY_CNT) {
                        continue;
                    }
                    if (groupingTarget == null
                            || robot.getPaintAmount() > groupingTarget.getPaintAmount()
                            || (robot.getPaintAmount() == groupingTarget.getPaintAmount() && robot.location.distanceSquaredTo(enemyHQLoc) < groupingTarget.location.distanceSquaredTo(enemyHQLoc))
                        ) {
                        groupingTarget = robot;
                    }
                    friendInCloud[friendlyMopperCnt] = rc.senseCloud(robot.location);
                    friendlyMoppers[friendlyMopperCnt++] = robot;
                    ourTeamStrength += 1;
                    if (robot.location.distanceSquaredTo(rc.getLocation()) <= 8){
                        closeFriendsSize++;
                    }
                } else if (robot.type == UnitType.SOLDIER && robot.getNumAnchors(Anchor.STANDARD) != 0) {
                    anchoringSoldier = robot.location;
                }
            } else {
                if (robot.type.isTowerType()) {
                    ...
                    if (enemyMopperCnt >= MAX_ENEMY_CNT) {
                        continue;
                    }
                    enemyInCloud[enemyMopperCnt] = rc.senseCloud(robot.location);
                    enemyMoppers[enemyMopperCnt++] = robot;
                    ourTeamStrength -= 1;
                    if (robot.location.distanceSquaredTo(rc.getLocation()) > ATTACK_DIS) {
                        chaseTarget = robot;
                    }
                }
                // } else if (robot.type == UnitType.LAUNCHER || robot.type == UnitType.DESTABILIZER) {
                //     if (enemyMopperCnt >= MAX_ENEMY_CNT) {
                //         continue;
                //     }
                //     enemyInCloud[enemyMopperCnt] = rc.senseCloud(robot.location);
    ...
    static void macro() throws GameActionException {
        ...
        // if (tryTowerStuff())  //LIKE TOWERS I THINK
        //     return;
...
//        }
        ...
        if (closeFriendsSize < 3 && (rc.getRoundNum() - lastMopperAttackRound) < 10) {
    ...
    /* static boolean tryTowerStuff() throws GameActionException {
        // attempt to go heal if less than 100 health, capture enemy island, protect friendly island
        boolean needHeal =  (rc.getPaintAmount() < MAX_HEALTH && state == HEALING) || rc.getPaintAmount() < HEALING_CUTOFF;
        if (state == HEALING && !needHeal && (ourTeamStrength >= 3 || rc.getRoundNum() - lastMopperAttackRound > 50)) {
            islandTargetIndex = 0;
...
//                rc.isMovementReady());
        ...
        if (target != null) {
            if (target == attackTarget) {
                lastMopperAttackRound = rc.getRoundNum();
            }
            ...
            for (int i = enemyMopperCnt; --i >= 0;) {
                RobotInfo enemy = enemyMoppers[i];
        ...
        if (rc.isMovementReady() && rc.isActionReady()) {
            if (chaseTarget != null) {
                ...
                cachedRound = rc.getRoundNum();
                if (rc.getPaintAmount() > chaseTarget.health || ourTeamStrength > 2 || chaseTarget.type != UnitType.LAUNCHER) {
    ...
    private static void chase(MapLocation location) throws GameActionException{
        ...
        for (Direction dir : dirs) {
            if (rc.canMove(dir) && rc.getLocation().add(dir).distanceSquaredTo(location) <= ATTACK_DIS) {
                ...
                for (int i = enemyMopperCnt; --i >= 0;){
                    int newDis = rc.getLocation().add(dir).distanceSquaredTo(enemyMoppers[i].location);
    ...
    private static RobotInfo getImmediatelyAttackableTarget() {
        ...
        RobotInfo rv = null;
        int minDis = Integer.MAX_VALUE;
        for (int enemy_i = enemyMopperCnt; --enemy_i >= 0;) {
            RobotInfo enemy = enemyMoppers[enemy_i];
            int dis = enemy.location.distanceSquaredTo(rc.getLocation());
            if (dis > ATTACK_DIS) {
                continue;
            }
            ...
            for (int friend_i = friendlyMopperCnt; --friend_i >= 0;) {
                int friendEnemyDis = friendlyMoppers[friend_i].location.distanceSquaredTo(enemy.location);
